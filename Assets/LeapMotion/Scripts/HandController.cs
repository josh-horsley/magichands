...
public class HandController : MonoBehaviour {
  ...
  void Start() {
      var gestureObj = GameObject.FindGameObjectWithTag("gesture");

      if (gestureObj != null)
          gestureTest = gestureObj.GetComponent<GestureTest>();
    ...
    for (int h = 0; h < num_hands; ++h) {
      ...
      if (model != null) {
        ...
        if (!all_hands.ContainsKey(leap_hand.Id)) {
          ...
          //set hand to proper gesture test hand
          if (leap_hand.IsLeft)
              gestureTest.SetLeftHand(new_hand.gameObject);
          if (leap_hand.IsRight)
              gestureTest.SetRightHand(new_hand.gameObject);
        }
        else {
